...
> **注意：** EMC 9.4.4 当前仅在服务器（Ubuntu, `/opt/emc-9.4.4/`）上安装。以下 CLI 输出来自服务器运行结果。本地 macOS 环境 EMC 待安装。

- **命令可用性：** ⬜（本地 macOS 环境尚未安装 EMC）
- **验证状态：** ⬜ 未运行（以下示例尚未在本地 macOS 环境实际执行）

| 项目 | 状态 |
|------|------|
| 服务器 EMC 二进制存在 | ✅ `/opt/emc-9.4.4/bin/emc_linux_x86_64` |
| 服务器版本确认 | ✅ v9.4.4 (Jul 21 2026) |
| 服务器 help 输出获取 | ✅ 完整 |
| 本地 macOS 实际运行 EMC | ⬜ 待验证 |
